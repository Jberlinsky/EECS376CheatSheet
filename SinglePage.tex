...
%TODO
  ...
  \subsection{CLIQUE}
  A clique is a subgraph wherein every two nodes are conected by an edge; a $k-$clique is a clique that contains $k$ nodes. Clique problems include:
  
  \begin{itemize}
      \item Finding the maximum clique (MAXCLIQUE)
      \item Finding a maximum weight clique in a weighted graph
      \item Listing all maximal cliques
      \item Deciding whether a graph contains a clique larger than a given size
  \end{itemize}

  \subsubsection{Decision Version}
  Is there a $k$-clique in a given graph G?

  NP-Complete. From a given CNF formula, form a graph that has a vertex for every pair $(v, c)$, where $v$ is a variable or its negation and $c$ is a clause in the formula that contains $v$. Vertices are connected by an edge if they represent compatible variable assignments for different clauses. If $k$ denotes the number of clauses in the CNF formula, the the $k-$vertex cliques in this graph represent ways of assigning truth values to some of its variables in order to satisfy the formula; therefore, the formula is satisfiable if and only if a $k-$vertex clique exists.
  ...
  \subsection{3SAT}
  A special instance of the satisfiability problem, written in {\bf 3CNF}-form: $\Phi = (x_1 \vee x_2 \vee x_3) \wedge (x_4 \vee x_5 \vee x_6) ...$.

  \subsection{VERTEX-COVER}
  For an undirected graph $G$, a vertex cover is a subset of nodes where every edge of $G$ touches one of those nodes.

  \subsection{3COL}
  Graph 3-colorability. NP-complete. See Homework section for an example.
  ...
  Whereas in the last section we were dealing with time, now we consider space as the number of cells the TM uses.
    ...
    \includegraphics[scale=0.25]{fig8_7.png}
  ...
  \subsection{2-approx for minVERTEX-COVER}
...
%\put(70,50){\makebox(0,0)[c]{$F$}}
    ...
    \subsection{HW8}
        \subsubsection{Specializing TQFB}
            {\bf Show that the restriction (special case) of TQBF in which the matrix
